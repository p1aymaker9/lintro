...
function buildFastStructPrompt(targetLang: string): string {
  return [
    ...
    `Respond ONLY with a valid JSON object (no markdown, no code fences, no extra text).`,
    `Output language MUST strictly follow target_lang (${langLabel(targetLang)}).`,
    ``,
    `JSON schema:`,
    `{`,
    ...
    ``,
    `Rules:`,
    `- Split the sentence into grammar units (3-10 chunks).`,
    `- Chunks MUST be contiguous.`,
    `- Chunks MUST be non-overlapping.`,
    ...
    ``,
    `Positive example 1 (GOOD):`,
    `Input: "I saw him yesterday."`,
    `Output: {"translation":"我昨天见到了他。","chunks":[{"text":"I","role":"[执行者]"},{"text":" saw","role":"[动作]"},{"text":" him","role":"[对象]"},{"text":" yesterday","role":"[时间]"},{"text":".","role":"[标点]"}]}`,
    ``,
    `Positive example 2 (GOOD):`,
    `Input: "She quickly finished her homework."`,
    `Output: {"translation":"她很快完成了作业。","chunks":[{"text":"She","role":"[执行者]"},{"text":" quickly","role":"[方式]"},{"text":" finished","role":"[动作]"},{"text":" her homework","role":"[对象]"},{"text":".","role":"[标点]"}]}`,
    ``,
    `Counter-example (BAD, do NOT do this):`,
    `Input: "I saw him yesterday."`,
    `Bad Output: {"translation":"我昨天见到了他。","chunks":[{"text":"I saw","role":"[动作]"},{"text":" him yesterday","role":"[对象]"},{"text":" yesterday.","role":"[时间]"}]}`,
    `Why bad: chunks overlap on "yesterday" and fail exact reconstruction with clean boundaries.`,
    '',
    JSON_ONLY_SUFFIX,
  ].join('\n');
}
